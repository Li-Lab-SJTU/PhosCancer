...
library(ggpubr)
if(!is.null(Total)){
jpeg(filename=paste0('/dssg/home/Projects/PhosCancer/Analysis/PNGs/Stagediffer/',Annotation[Annotation[,1]==whichsite,4],'_',Annotation[Annotation[,1]==whichsite,3],'_ALL.jpeg'),width = 1000, height = 600)
print(ggplot(Total,aes(Cancertype,Site,color = Stage)) + stat_boxplot(mapping=aes(x=Cancertype,y=Site),geom='errorbar',size=0.8)+
