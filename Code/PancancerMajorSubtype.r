...
library(ggpubr)
if(!is.null(Total)){
jpeg(filename=paste0('/dssg/home/acct-clslj/clslj-2/Dongqun/Projects/PhosCancer/Analysis/PNGs/MajorSubtypediffer/',Annotation[Annotation[,1]==whichsite,4],'_',Annotation[Annotation[,1]==whichsite,3],'_ALL.jpeg'),width = 1000, height = 600)
print(ggplot(Total,aes(Cancertype,Site,color = Subtype)) + stat_boxplot(mapping=aes(x=Cancertype,y=Site),geom='errorbar',size=0.8)+# stat_boxplot() 函数用来给箱子添加median ±1.5 IQR 的最大值和最小值的bar, 注意分组用的参数的设置position,根据图像大小设置bar间距width参数
