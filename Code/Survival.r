...
splots<-list()
#if(na.omit(any(NormPhosphoGroupsfilteredOS[-which(is.na(NormPhosphoGroupsfilteredOS[,6])),4])==1)){
#if(!all(is.na(NormPhosphoGroupsfilteredOS[,3]))&any(na.omit(NormPhosphoGroupsfilteredOS[,c(3,4,6)])[,2]==1)){
...
splots[[1]]=ggsurvplot(fit,data = OSres.cut,size = 2.0,
				...
				tables.theme=theme_cleantable() ,# 作用于生存表的ggplot2主题名称有theme_survminer、theme_cleantable()
                ...
                font.legend  = c(16, "bold", "black")# 图例字体
 )
...
#if(any(na.omit(NormPhosphoGroupsfilteredDFS[-which(is.na(NormPhosphoGroupsfilteredDFS[,6])),2])==1)){
#if(!all(is.na(NormPhosphoGroupsfilteredDFS[,2]))&any(na.omit(NormPhosphoGroupsfilteredDFS[,c(1,2,6)])[,2]==1)){
...
splots[[4]]=ggsurvplot(fit,data = NormPhosphoGroupsfilteredDFS,size = 2.0,
                palette = c('#ED0000','#183F7D'),#c('#ED0000','#183F7D'), c("#E7B800", "#2E9FDF") npg,lancet,
                conf.int = F,conf.int.style='ribbon', conf.int.alpha=0.1,
                pval = T,pval.method = T,censor.shape="+",pval.size=8,pval.method.size=8,
                risk.table = T,risk.table.pos='in', risk.table.fontsize=6,
                #legend=c(0.85,0.85),
				...
				legend = 'right',
				legend.labs=c('high','low'),
				tables.theme=theme_cleantable(risk.table.height = 0.5) ,# 作用于生存表的ggplot2主题名称有theme_survminer、theme_cleantable()
...
#if(!all(is.na(NormPhosphoGroupsfilteredOS[,3]))&!all(is.na(NormPhosphoGroupsfilteredOS[,2]))){
if(length(which(na.omit(NormPhosphoGroupsfiltered[,c(3,4,6)])[,2]==1))>1&length(which(na.omit(NormPhosphoGroupsfiltered[,c(1,2,6)])[,2]==1))>1){
 jpeg(filename=paste0('/dssg/home/acct-clslj/clslj-2/Dongqun/Projects/PhosCancer/Analysis/PNGs/Surdiffer/',Annotation[Annotation[,1]==whichsite,4],'_',Annotation[Annotation[,1]==whichsite,3],'_',Cancertype,'.jpeg'),width = 1100, height = 800)
 print(arrange_ggsurvplots(splots, print = TRUE,ncol = 2, nrow = 2))
 dev.off()} 
if(length(which(na.omit(NormPhosphoGroupsfiltered[,c(3,4,6)])[,2]==1))>1&length(which(na.omit(NormPhosphoGroupsfiltered[,c(1,2,6)])[,2]==1))<=1){
 jpeg(filename=paste0('/dssg/home/acct-clslj/clslj-2/Dongqun/Projects/PhosCancer/Analysis/PNGs/Surdiffer/',Annotation[Annotation[,1]==whichsite,4],'_',Annotation[Annotation[,1]==whichsite,3],'_',Cancertype,'.jpeg'),width = 1100, height = 400)
