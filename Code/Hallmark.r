...
gsva_matrix <- read.table(paste0("/dssg/home/acct-clslj/clslj-2/Dongqun/Projects/PhosCancer/Proteome/Proteome_gsva/",Cancertype,".tsv"),comment.char="",quote='',sep='\t',header = TRUE,fill=T,check.names=F,stringsAsFactors = FALSE)
...
#png(filename=paste0('/dssg/home/acct-clslj/clslj-2/Dongqun/Projects/PhosCancer/Analysis/PNGs/HallmarkCor/',Annotation[Annotation[,1]==whichsite,4],'_',Annotation[Annotation[,1]==whichsite,3],'_',Cancertype,'.png'),width = 800, height = 800)
jpeg(filename=paste0('/dssg/home/acct-clslj/clslj-2/Dongqun/Projects/PhosCancer/Analysis/PNGs/HallmarkCor/',Annotation[Annotation[,1]==whichsite,4],'_',Annotation[Annotation[,1]==whichsite,3],'_',Cancertype,'.jpeg'),width = 800, height = 800)
print(ggplot(bardata,aes(x=as.factor(id),y=Cor,fill=Correlation))+
  geom_bar(stat="identity")+
  coord_polar()+  scale_y_continuous(limits = c(limitsstart, max(abs(as.numeric(corE)))+0.1), expand = c(-0.1, 0)) +#ylim(-0.1,max(abs(as.numeric(corE)))+0.1)+
  ...
  scale_fill_manual(values = c('#9FB3D2','#BE7F7D')))# +scale_y_continuous(labels=c('','B','C','D','E'),limits=c(-0.1, max(abs(as.numeric(corE)))+0.1))
dev.off()
...
write.table(HallmarkCorTop,file=paste0("/dssg/home/acct-clslj/clslj-2/Dongqun/Projects/PhosCancer/Analysis/Quantitative/",Cancertype,"_HallmarkCorTop.tsv"),sep="\t",quote=F,row.names=F,col.names=T)
write.table(HallmarkCorTotal,file=paste0("/dssg/home/acct-clslj/clslj-2/Dongqun/Projects/PhosCancer/Analysis/Quantitative/",Cancertype,"_HallmarkCorTotal.tsv"),sep="\t",quote=F,row.names=F,col.names=T)
